...
// Listeners iniciales
chrome.storage.local.get(['scroll_ig', 'shortcuts_enabled', 'pip_enabled'], (result) => {
  // Nota: Usamos scroll_ig (asumiremos que existe o lo añadimos a popup.js luego)
  if (result.scroll_ig) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'toggleAutoScroll' && request.platform === 'ig') {
...
function injectPipButton() {
  ...
  btn.onclick = async () => {
    let mainVideo = getActiveIgVideo();
    if (mainVideo) {
  ...
  if (!window.pipWatcherInterval) {
    window.pipWatcherInterval = setInterval(() => {
      if (document.pictureInPictureElement) {
        ...
        let newActiveVideo = getActiveIgVideo();
...
function getActiveIgVideo() {
  let videos = Array.from(document.querySelectorAll('video'));
  return videos.find(v => v.offsetHeight > 100 && !v.paused) || videos[0];
}
...
function handleShortcuts(e) {
  chrome.storage.local.get(['shortcuts_enabled'], (result) => {
    if(result.shortcuts_enabled && window.location.href.includes('/reels/')) {
      if(e.code === 'Space') {
        let video = getActiveIgVideo();
        if(video) {
...
function initAutoScroll() {
  ...
  isInitialized = true;
  console.log("Auto Scroll Pro: Instagram Reels inicializado");
      ...
      if (isVisible && !video.dataset.autoScrollListener) {
        ...
        video.addEventListener('timeupdate', () => {
          ...
          if ((isNearEnd || hasLooped) && !video.dataset.isScrolling) {
            ...
            console.log("Auto Scroll Pro: IG Reel terminado o en loop, haciendo scroll...");
            try { chrome.runtime.sendMessage({ action: 'incrementScroll', platform: 'ig' }); } catch(e) {}
